...
extern kmem_cache_t *proc_cache;
...
procinit(void)
{
  ...
  */
  for (int i = 0; i < NPROC; i++)
  {
    proc[i] = kmem_cache_alloc(proc_cache);
    initlock(&proc[i]->lock, "proc");
    proc[i]->state = UNUSED;
    proc[i]->kstack = KSTACK(i);
  }
 /* for (int i = 0; i < NPROC; i++) {
    kmem_cache_free(proc_cache, proc[i];
  }
*/
}
    ...
    for(int i = 0; i < NPROC; i++){
      if(proc[i]->parent == p){
        ...
        if(proc[i]->state == ZOMBIE){
          // Found one.
          pid = proc[i]->pid;
          if(addr != 0 && copyout(p->pagetable, addr, (char *)&proc[i]->xstate,
 ...
 // struct proc *p;
  ...
  for(int i = 0; i < NPROC; i++) {
    ...
    if(proc[i]->state >= 0 && proc[i]->state < NELEM(states) && states[proc[i]->state])
      state = states[proc[i]->state];
    else
      state = "???";
    printf("%d %s %s", proc[i]->pid, state, proc[i]->name);
    printf("\n");
  }
}
